...
/// @notice Contract for locking ROCK tokens for a period of time.
...
contract TokenLocker is Ownable {
    ...
    function setAddresses(IERC20 _rock) external onlyOwner {
        ROCK = _rock;
        ...
        returns (uint256[2][] memory lockData)
    {
        ...
        for (uint256 i = week + 1; i < week + MAX_LOCK_WEEKS + 1;) {
    ...
    /**
        ...
        @param _amount Amount of ROCK to lock. This balance transfered from the caller.
    ...
    function lock(
        ...
        ROCK.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    /**
        ...
        @param _amount Amount of ROCK to extend the lock for. When the value given equals
    ...
    function withdrawExitStream() external returns (bool) {
        ...
        if (stream.start > 0) {
            ...
            ROCK.safeTransfer(msg.sender, amount);
        }
    ...
    /**
        @notice Get the amount of ROCK in expired locks that is
    ...
    /**
        @notice Get the amount of ROCK available to withdraw
